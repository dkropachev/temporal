...
// hsmCompletionToken / chasmCompletionToken build valid framework-shaped tokens via the owner
// helpers, so these tests never hard-code either framework's ref layout.
func hsmCompletionToken() *tokenspb.NexusOperationCompletion {
	return nexusoperations.CompletionFromOperationIdentity(testOperationIdentity())
}

func chasmCompletionToken(t *testing.T) *tokenspb.NexusOperationCompletion {
	t.Helper()
	completion, err := chasmworkflow.CompletionFromOperationIdentity(testOperationIdentity())
