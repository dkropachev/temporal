...
//
// The parsing and building of each framework's ref is owned by that framework
// (components/nexusoperations for HSM, chasm/lib/workflow for CHASM); this only routes between them
// via the shared nexus.OperationIdentity. Versioned-transition fields are not carried across —
// identity is re-established by request ID, matching the completion handlers' reset fallback.
func convertCompletionToOtherFramework(completion *tokenspb.NexusOperationCompletion) (*tokenspb.NexusOperationCompletion, error) {
	if len(completion.GetComponentRef()) > 0 {
		// CHASM token -> HSM.
		identity, err := chasmworkflow.OperationIdentityFromCompletion(completion)
		if err != nil {
			return nil, err
		}
		return nexusoperations.CompletionFromOperationIdentity(identity), nil
	}
	// HSM token -> CHASM.
	identity, err := nexusoperations.OperationIdentityFromCompletion(completion)
	if err != nil {
		return nil, err
	}
	return chasmworkflow.CompletionFromOperationIdentity(identity)
}
