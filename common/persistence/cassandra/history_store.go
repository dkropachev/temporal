...
func (h *HistoryStore) executeOptionalHistoryNodeMutation(
	ctx context.Context,
	plan historyNodeMutationPlan,
	args ...any,
) (bool, error) {
	if plan.optionalMirrorTable == "" {
		return false, nil
	}
	if err := h.Session.Query(plan.primaryQuery, args...).WithContext(ctx).Exec(); err != nil {
		return true, err
	}
	err := h.Session.Query(plan.mirrorQuery, args...).WithContext(ctx).Exec()
	if gocql.IsUnconfiguredTableError(err, plan.optionalMirrorTable) {
		return true, nil
	}
	return true, err
}
...
func (h *HistoryStore) DeleteHistoryNodes(
	...
	if handled, err := h.executeOptionalHistoryNodeMutation(
		ctx,
		plan,
		treeID,
		branchID,
...
func (h *HistoryStore) DeleteHistoryBranch(
	...
	for _, br := range request.BranchRanges {
		if handled, optionalErr := h.executeOptionalHistoryNodeMutation(
			ctx,
			plan,
			...
			if optionalErr != nil {
				return gocql.ConvertError("DeleteHistoryBranch", optionalErr)
			}
			continue
		}
