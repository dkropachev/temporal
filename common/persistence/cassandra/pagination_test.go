...
func TestListQueuesDoesNotAutoFetchPastRequestedPage(t *testing.T) {
	queueBytes, err := (&persistencespb.Queue{
		Partitions: map[int32]*persistencespb.QueuePartition{
...
func TestOptionalHistoryNodeDeleteIgnoresMissingMirror(t *testing.T) {
	session := &recordingSession{
		t: t,
		queryFn: func(stmt string, _ ...any) cgocql.Query {
			return &recordingQuery{execFn: func() error {
				if stmt == v2templateDeleteHistoryNodeV2 {
					return historyNodeRequestError{
						code:    gocql.ErrCodeInvalid,
						message: "unconfigured table history_node_v2",
					}
				}
	...
	handled, err := store.executeOptionalHistoryNodeMutation(
		t.Context(),
		plan,
		"tree-id",
		"branch-id",
...
func (i *recordingIter) NumRows() int {
	if i.currentPageRows > 0 {
		return i.currentPageRows
	}
