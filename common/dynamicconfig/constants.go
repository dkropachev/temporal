...
It clamps the recorded TweakablePolicies.Version to min(current, N) for N in [0, the highest version]; a negative value (the default) disables the clamp and values above the highest version are a no-op. The ceiling is read once, when a worker starts running a schedule, and then held fixed for that execution.`,
	)
