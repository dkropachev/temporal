...
Matching Service splits Task Queues into partitions to provide higher throughput overall. Reads and writes default to 4 partitions, while selected system task queues use 1, and both counts are configurable. Partition ownership can be reassigned, and partitions' metadata and task backlog can be loaded/unloaded from storage.
